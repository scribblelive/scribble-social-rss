...
$request = $client->get('/event/' . $EventId . '/all?Max=20');
...
foreach( $json["Posts"] as $Post )
{
    ...
    try
    {
        ...
        if( false && ( $Post["Type"] != "IMAGE" || ( isset( $_GET["notweets"] ) && $_GET["notweets"] == "1" ) ) && preg_match( "/twitter\.com/", $Post["Source"] ) )
        {
            continue;
        }
        else if( $Post["Type"] == "TEXT" && ! empty( $TextContent )  )
        {
            ...
            if( strlen( $Content ) + strlen( $TextContent ) > 140 && false )
            {
        ...
        else if( $Post["Type"] == "IMAGE" )
        {
            ...
            if( isset( $TextContent ) )
            {
                ...
                if( strlen( $Content ) + strlen( $TextContent ) > 140 - 26 && false )
                {
                    $Content = $Content . ShortenText( $TextContent, 140 - 26 - strlen( $Content ) - 3 ) . "... " ;//. $PostUrl;
                    $IsShortened = true;
                    $Content = preg_replace( "/\"/", "\\\"", html_entity_decode( $Content, 0, 'UTF-8' ) );
